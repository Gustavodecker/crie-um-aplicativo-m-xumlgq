...
export function registerInitRoutes(app: App) {
  ...
  // POST /api/init/mother - Register as mother for a baby
  app.fastify.post('/api/init/mother', {
    schema: {
      description: 'Register as mother for a baby',
      tags: ['init'],
      body: {
        type: 'object',
        required: ['babyId'],
  ...
  }, async (request: FastifyRequest<{ Body: { babyId: string } }>, reply: FastifyReply) => {
    const session = await requireAuth(request, reply);
    if (!session) return;

    app.logger.info({ userId: session.user.id, babyId: request.body.babyId }, 'Registering as mother');
    ...
    if (baby.motherUserId) {
      app.logger.warn({ babyId: request.body.babyId }, 'Baby already has a registered mother');
      return reply.status(400).send({ error: 'Baby already has a registered mother' });
    }

    if (baby.motherEmail !== session.user.email) {
      app.logger.warn({ userId: session.user.id, babyId: request.body.babyId, email: session.user.email, expectedEmail: baby.motherEmail }, 'Email mismatch');
