...
function generatePDFHTML(reports: DailyReport[], babyName: string): string {
  ...
  const leftColumnReports: DailyReport[] = [];
  const rightColumnReports: DailyReport[] = [];
    ...
    <html>
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background: #ffffff; padding: 8px; font-size: 8px; line-height: 1.2; color: #333; }
          .header { text-align: center; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 2px solid ${primaryColor}; }
          .header h1 { color: ${primaryColor}; font-size: 16px; margin-bottom: 3px; font-weight: 700; }
          .header p { color: #666; font-size: 9px; margin: 2px 0; }
          .columns-container { display: flex; gap: 8px; justify-content: space-between; }
          .column { width: 49.5%; display: flex; flex-direction: column; gap: 8px; }
          .card { background: #ffffff; border: 1px solid #e0e0e0; border-radius: 6px; padding: 8px; border-left: 3px solid ${primaryColor}; page-break-inside: avoid; break-inside: avoid; }
          .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; padding-bottom: 4px; border-bottom: 1px solid #e0e0e0; }
          .day-title { font-size: 11px; font-weight: bold; color: ${primaryColor}; margin: 0; }
          .date-text { font-size: 8px; color: #666; font-weight: 600; }
          .summary-box { background: ${primaryColor}10; border: 1px solid ${primaryColor}30; border-radius: 4px; padding: 5px; margin-bottom: 6px; }
          .summary-table { width: 100%; border-collapse: collapse; }
          .summary-table td { padding: 2px 4px; font-size: 8px; }
          .summary-label { color: #555; }
          .summary-value { text-align: right; font-weight: bold; color: ${primaryColor}; }
          .summary-total td { border-top: 1px solid ${primaryColor}30; padding-top: 3px; margin-top: 2px; }
          .section { margin-top: 6px; }
          .section-title { font-size: 9px; font-weight: bold; color: ${primaryColor}; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.3px; }
          .data-table { width: 100%; border-collapse: collapse; margin-top: 3px; font-size: 7px; }
          .data-table th { background: ${primaryColor}15; color: ${primaryColor}; font-weight: bold; padding: 3px 4px; text-align: left; border: 1px solid ${primaryColor}30; font-size: 7px; }
          .data-table td { padding: 3px 4px; border: 1px solid #e0e0e0; vertical-align: top; }
          .data-table tbody tr:nth-child(even) { background: #f9f9f9; }
          .detail-item { margin-bottom: 2px; line-height: 1.3; }
          .observation-box { background: #f5f5f5; border-left: 2px solid #999; border-radius: 3px; padding: 4px; margin-top: 3px; font-size: 7px; line-height: 1.3; }
          .observation-box.consultant { border-left-color: ${primaryColor}; background: ${primaryColor}08; }
      ...
      <body>
        ...
        <div class="columns-container">
          <div class="column">${leftColumnHTML}</div>
          <div class="column">${rightColumnHTML}</div>
        </div>
      </body>
    </html>
  `;
...
export default function AcompanhamentoScreen() {
  ...
  const handleBack = useCallback(() => {
    console.log('[Acompanhamento] Back button pressed');
    router.back();
  }, [router]);
