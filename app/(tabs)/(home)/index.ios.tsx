...
export default function ConsultantDashboardScreen() {
  ...
  const handleSaveComments = async () => {
    ...
    try {
      if (editingComments === "consultantComments") {
        ...
        setSelectedRoutine({ ...selectedRoutine, consultantComments: commentText });
  ...
  const handleSaveNapComments = async (napId: string) => {
    if (!selectedRoutine) return;
    try {
      await apiPut(`/api/naps/${napId}`, { consultantComments: commentText });
      const updatedNaps = selectedRoutine.naps?.map((nap) =>
  ...
  const handleUpdateNightSleep = async (field: string, value: string | null) => {
    ...
    try {
      await apiPut(`/api/night-sleep/${selectedRoutine.nightSleep.id}`, { [field]: value });
      setSelectedRoutine({
        ...selectedRoutine,
  ...
  const handleSaveNightComments = async () => {
    ...
    try {
      ...
      setSelectedRoutine({
        ...selectedRoutine,
        nightSleep: { ...selectedRoutine.nightSleep, consultantComments: commentText },
      });
      setEditingComments(null);
      setCommentText("");
    } catch (error) {
  ...
  const applyTimeSelection = (date: Date) => {
    ...
    console.log("[index.ios] Time confirmed:", timeString, "field:", timePickerField);
  ...
  if (selectedRoutine) {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <Stack.Screen options={{ title: `Rotina - ${selectedBaby?.name}`, headerShown: true }} />
        <ScrollView
            ...
            {selectedRoutine.naps && selectedRoutine.naps.length > 0 && (
              <View style={styles.section}>
                ...
                {selectedRoutine.naps.map((nap, index) => {
                  ...
                  const showResults = windowStart && nap.startTryTime && nap.fellAsleepTime && nap.wakeUpTime;
                  const janelaRealizada = showResults ? diffTime(windowStart, nap.startTryTime) : "";
                  const tempoAdormecer = showResults ? diffTime(nap.startTryTime, nap.fellAsleepTime) : "";
                  const duracaoSono = showResults ? diffTime(nap.fellAsleepTime, nap.wakeUpTime) : "";

                  return (
                    <View key={nap.id} style={styles.napCard}>
                      <View style={styles.napHeader}>
                        <Text style={styles.napTitle}>Soneca {nap.napNumber}</Text>
                        <TouchableOpacity onPress={() => handleDeleteNap(nap.id, nap.napNumber)} style={styles.deleteButton}>
                          <IconSymbol ios_icon_name="trash" android_material_icon_name="delete" size={18} color={colors.error} />
                        </TouchableOpacity>
                      </View>

                      <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker(`nap-${nap.id}-startTryTime`, nap.startTryTime)}>
                        <Text style={styles.timeLabel}>Início tentativa::</Text>
                        <Text style={styles.timeValue}>{nap.startTryTime || "Não informado"}</Text>
                      </TouchableOpacity>

                      <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker(`nap-${nap.id}-fellAsleepTime`, nap.fellAsleepTime)}>
                        <Text style={styles.timeLabel}>Dormiu às:</Text>
                        <Text style={styles.timeValue}>{nap.fellAsleepTime || "Não informado"}</Text>
                      </TouchableOpacity>

                      <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker(`nap-${nap.id}-wakeUpTime`, nap.wakeUpTime)}>
                        ...
                        {editingComments === `nap-${nap.id}` ? (
                          <View>
                            ...
                            <View style={styles.commentButtons}>
                              <TouchableOpacity style={styles.saveButton} onPress={() => handleSaveNapComments(nap.id)}>
                                <Text style={styles.saveButtonText}>Salvar</Text>
                              </TouchableOpacity>
                              <TouchableOpacity style={styles.cancelButton} onPress={() => { setEditingComments(null); setCommentText(""); }}>
                                <Text style={styles.cancelButtonText}>Cancelar</Text>
                              </TouchableOpacity>
                            </View>
                          </View>
                        ) : (
                          <TouchableOpacity onPress={() => handleEditComments(`nap-${nap.id}`, nap.consultantComments)}>
                            <Text style={styles.commentsText}>{nap.consultantComments || "Toque para adicionar comentários"}</Text>
                          </TouchableOpacity>
                        )}
                      </View>
                    </View>
            ...
            {selectedRoutine.nightSleep && (
              <View style={styles.section}>
                ...
                <View style={styles.napCard}>
                  <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker("night-startTryTime", selectedRoutine.nightSleep?.startTryTime || null)}>
                    <Text style={styles.timeLabel}>Início tentativa:</Text>
                    <Text style={styles.timeValue}>{selectedRoutine.nightSleep.startTryTime || "Não informado"}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker("night-fellAsleepTime", selectedRoutine.nightSleep?.fellAsleepTime || null)}>
                    <Text style={styles.timeLabel}>Dormiu às:</Text>
                    <Text style={styles.timeValue}>{selectedRoutine.nightSleep.fellAsleepTime || "Não informado"}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.timeRow} onPress={() => openTimePicker("night-finalWakeTime", selectedRoutine.nightSleep?.finalWakeTime || null)}>
                    <Text style={styles.timeLabel}>Acordou às:</Text>
                    <Text style={styles.timeValue}>{selectedRoutine.nightSleep.finalWakeTime || "Não informado"}</Text>
                  </TouchableOpacity>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Dormiu como:</Text>
                    <Text style={styles.infoValue}>{selectedRoutine.nightSleep.sleepMethod || "Não informado"}</Text>
                  </View>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Ambiente:</Text>
                    <Text style={styles.infoValue}>{selectedRoutine.nightSleep.environment || "Não informado"}</Text>
                  </View>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Como acordou:</Text>
                    <Text style={styles.infoValue}>{selectedRoutine.nightSleep.wakeUpMood || "Não informado"}</Text>
                  </View>
                  {selectedRoutine.nightSleep.observations && (
                    <View style={styles.observationsBox}>
                      <Text style={styles.observationsLabel}>Observações da Mãe:</Text>
                      <Text style={styles.observationsText}>{selectedRoutine.nightSleep.observations}</Text>
                    </View>
                  )}
                  <View style={styles.commentsSection}>
                    <Text style={styles.commentsLabel}>Comentários da Consultora:</Text>
                    {editingComments === "nightSleep" ? (
                      <View>
                        ...
                        />
                        <View style={styles.commentButtons}>
                          <TouchableOpacity style={styles.saveButton} onPress={handleSaveNightComments}>
                            <Text style={styles.saveButtonText}>Salvar</Text>
                          </TouchableOpacity>
                          <TouchableOpacity style={styles.cancelButton} onPress={() => { setEditingComments(null); setCommentText(""); }}>
                            <Text style={styles.cancelButtonText}>Cancelar</Text>
                      ...
                      <TouchableOpacity onPress={() => handleEditComments("nightSleep", selectedRoutine.nightSleep?.consultantComments || null)}>
                        <Text style={styles.commentsText}>{selectedRoutine.nightSleep.consultantComments || "Toque para adicionar comentários"}</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
                      ...
                      return (
                        <View key={waking.id} style={styles.wakingCard}>
                          ...
                          <View style={styles.infoRow}>
                            ...
                            <Text style={styles.infoValue}>{waking.startTime} - {waking.endTime}</Text>
                          </View>
                          <View style={styles.infoRow}>
              ...
              {editingComments === "consultantComments" ? (
                <View>
                  ...
                  <View style={styles.commentButtons}>
                    ...
                    <TouchableOpacity style={styles.cancelButton} onPress={() => { console.log("🔴 [Cancel Comments] Canceling edit"); setEditingComments(null); setCommentText(""); }}>
                      <Text style={styles.cancelButtonText}>Cancelar</Text>
                    </TouchableOpacity>
                  </View>
                </View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => { console.log("🟢 [General Comments] Tapped to edit general comments"); handleEditComments("consultantComments", selectedRoutine.consultantComments); }}
                  activeOpacity={0.7}
                >
                  <Text style={styles.commentsText}>{selectedRoutine.consultantComments || "Toque para adicionar comentários"}</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
  ...
  if (selectedBaby) {
    return (
      <SafeAreaView style={styles.container}>
        <Stack.Screen options={{ title: `Rotinas - ${selectedBaby.name}`, headerShown: true }} />
        <ScrollView
          style={styles.scrollView}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}
        >
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.babyInfo}>{selectedBaby.ageMonths} meses e {selectedBaby.ageDays} dias</Text>
            ...
            {selectedBaby.activeContract && (
              <View style={styles.contractInfo}>
                ...
                <Text style={styles.contractDetail}>Início: {formatDateToBR(selectedBaby.activeContract.startDate)}</Text>
                <Text style={styles.contractDetail}>Duração: {selectedBaby.activeContract.durationDays} dias</Text>
                <View style={styles.contractStatusBadge}>
                  <Text style={styles.contractStatusText}>
                    {selectedBaby.activeContract.status === "active" ? "Vigente" : selectedBaby.activeContract.status === "paused" ? "Em Pausa" : "Concluído"}
                  </Text>
                </View>
              </View>
            )}
          ...
          <View style={styles.actionButtons}>
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: "/contract-details", params: { babyId: selectedBaby.id, babyName: selectedBaby.name } })}
            >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                router.push({ pathname: "/(tabs)/(home)/acompanhamento", params: { babyId: selectedBaby.id, babyName: selectedBaby.name } });
              }}
            >
              <IconSymbol ios_icon_name="chart.bar" android_material_icon_name="assessment" size={24} color={colors.primary} />
          ...
          {loadingRoutines ? (
            ...
            <View style={styles.routinesList}>
              ...
              {routines.map((routine) => {
                ...
                return (
                  <TouchableOpacity key={routine.id} style={styles.routineCard} onPress={() => handleSelectRoutine(routine)}>
                    ...
                    <Text style={styles.routineInfo}>Acordou: {routine.wakeUpTime || "Não informado"}</Text>
                    <Text style={styles.routineInfo}>Sonecas: {routine.naps?.length || 0}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
  ...
  const renderBabyCard = (baby: Baby) => {
    ...
    if (baby.activeContract) {
      ...
      if (status === "active") { contractStatus = "Vigente"; contractColor = colors.success; }
      else if (status === "paused") { contractStatus = "Em Pausa"; contractColor = colors.warning; }
      else if (status === "completed") { contractStatus = "Concluído"; contractColor = colors.textSecondary; }
    }
          ...
          <View style={styles.babyDetails}>
            ...
            <View style={styles.babyCardFooter}>
              ...
              <View style={[styles.contractBadge, { backgroundColor: contractColor + "20" }]}>
                <Text style={[styles.contractBadgeText, { color: contractColor }]}>{contractStatus}</Text>
              </View>
            </View>
          </View>
          <IconSymbol ios_icon_name="chevron.right" android_material_icon_name="chevron-right" size={24} color={colors.textSecondary} />
          ...
          {!baby.archived && (
            ...
            >
              {isLoadingAction ? (
                ...
                <IconSymbol ios_icon_name="archivebox" android_material_icon_name="archive" size={16} color={colors.primary} />
              )}
          ...
          {baby.archived && (
            ...
            >
              {isLoadingAction ? (
                ...
                <IconSymbol ios_icon_name="arrow.uturn.up.circle" android_material_icon_name="unarchive" size={16} color={colors.success} />
              )}
    ...
    <SafeAreaView style={styles.container}>
      <Stack.Screen
        options={{
          ...
          headerShown: true,
          headerRight: () => (
            <TouchableOpacity onPress={() => router.push('/edit-consultant-profile')} style={{ marginRight: spacing.md }}>
              <IconSymbol ios_icon_name="person.circle" android_material_icon_name="account-circle" size={28} color={colors.primary} />
            </TouchableOpacity>
          ),
        }}
      />
      ...
      >
        ...
        <TouchableOpacity style={styles.registerButton} onPress={handleRegisterBaby} activeOpacity={0.7}>
          <IconSymbol ios_icon_name="plus.circle.fill" android_material_icon_name="add-circle" size={24} color={colors.card} />
          <Text style={styles.registerButtonText}>Cadastrar Novo Bebê</Text>
        </TouchableOpacity>

        <View style={styles.sectionHeader}>
          <IconSymbol ios_icon_name="arrow.2.circlepath" android_material_icon_name="sync" size={16} color={colors.primary} />
        ...
        {activeBabies.length === 0 ? (
          ...
          <View style={styles.babiesList}>{activeBabies.map(renderBabyCard)}</View>
        )}

        <TouchableOpacity
          style={[styles.sectionHeader, styles.sectionHeaderArchived]}
          onPress={() => { const next = !archivedSectionExpanded; console.log(`📂 [Arquivados] Section toggled: ${next ? 'expanded' : 'collapsed'}`); setArchivedSectionExpanded(next); }}
          activeOpacity={0.7}
        >
          <IconSymbol ios_icon_name="archivebox" android_material_icon_name="archive" size={16} color={colors.textSecondary} />
        ...
        {archivedSectionExpanded && (
          archivedBabies.length === 0 ? (
            ...
            <View style={styles.babiesList}>{archivedBabies.map(renderBabyCard)}</View>
          )
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  babyCountText: { fontSize: 14, color: colors.textSecondary },
  sectionHeader: { flexDirection: "row", alignItems: "center", paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, marginTop: spacing.md, marginBottom: spacing.xs, gap: spacing.sm },
  sectionHeaderArchived: { marginTop: spacing.xl },
  sectionHeaderText: { fontSize: 13, fontWeight: '700', color: colors.primary, textTransform: "uppercase", letterSpacing: 0.8, flex: 1 },
  sectionHeaderTextArchived: { color: colors.textSecondary },
  sectionBadge: { backgroundColor: colors.primary + "20", paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: borderRadius.sm, minWidth: 24, alignItems: "center" },
  sectionBadgeArchived: { backgroundColor: colors.border },
  sectionBadgeText: { fontSize: 12, fontWeight: '700', color: colors.primary },
  sectionBadgeTextArchived: { color: colors.textSecondary },
  ...
  babyDetails: { flex: 1 },
  babyCardName: { fontSize: 17, fontWeight: "700", color: colors.text, marginBottom: 2 },
  babyCardInfo: { fontSize: 13, color: colors.textSecondary, marginBottom: 6 },
  babyCardFooter: { flexDirection: "row", alignItems: "center", gap: spacing.sm },
  ageContainer: { flexDirection: "row", alignItems: "center", gap: 4 },
  ageText: { fontSize: 12, color: colors.textSecondary },
  contractBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: borderRadius.sm },
  contractBadgeText: { fontSize: 11, fontWeight: "600" },
  babyCardActions: { flexDirection: "row", borderTopWidth: 1, borderTopColor: colors.border },
  archiveButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: spacing.sm, gap: 4, borderRightWidth: 1, borderRightColor: colors.border },
  archiveButtonText: { fontSize: 13, color: colors.primary, fontWeight: "600" },
  unarchiveButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: spacing.sm, gap: 4, borderRightWidth: 1, borderRightColor: colors.border },
  unarchiveButtonText: { fontSize: 13, color: colors.success, fontWeight: "600" },
  deleteBabyButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: spacing.sm, gap: 4 },
  deleteBabyButtonText: { fontSize: 13, color: colors.error, fontWeight: "600" },
  emptyContainer: { padding: spacing.xl, alignItems: "center" },
  emptyText: { fontSize: 16, color: colors.textSecondary, textAlign: "center" },
  card: { backgroundColor: colors.card, margin: spacing.lg, padding: spacing.lg, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border },
  babyName: { fontSize: 22, fontWeight: "700", color: colors.text, marginBottom: spacing.sm },
  babyInfo: { fontSize: 15, color: colors.textSecondary, marginBottom: spacing.xs },
  contractInfo: { marginTop: spacing.md, padding: spacing.md, backgroundColor: colors.primary + "10", borderRadius: borderRadius.md },
  contractHeader: { flexDirection: "row", alignItems: "center", gap: spacing.sm, marginBottom: spacing.sm },
  contractTitle: { fontSize: 16, fontWeight: "700", color: colors.primary },
  contractDetail: { fontSize: 14, color: colors.text, marginBottom: spacing.xs },
  contractStatusBadge: { marginTop: spacing.xs, alignSelf: "flex-start", backgroundColor: colors.success + "20", paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: borderRadius.sm },
  contractStatusText: { fontSize: 12, fontWeight: "600", color: colors.success },
  actionButtons: { flexDirection: "row", paddingHorizontal: spacing.lg, gap: spacing.md, marginBottom: spacing.md },
  actionButton: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: colors.card, paddingVertical: spacing.md, borderRadius: borderRadius.lg, gap: spacing.sm, borderWidth: 1, borderColor: colors.border },
  actionButtonText: { fontSize: 14, fontWeight: "600", color: colors.primary },
  routinesList: { paddingHorizontal: spacing.lg },
  listTitle: { fontSize: 18, fontWeight: "700", color: colors.text, marginBottom: spacing.md },
  routineCard: { backgroundColor: colors.card, borderRadius: borderRadius.lg, padding: spacing.md, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.border },
  routineHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.xs },
  routineDayOfWeek: { fontSize: 13, fontWeight: "700", color: colors.primary, textTransform: "uppercase" },
  routineDate: { fontSize: 16, fontWeight: "600", color: colors.text },
  routineInfo: { fontSize: 14, color: colors.textSecondary, marginTop: 2 },
  dayOfWeek: { fontSize: 13, fontWeight: "700", color: colors.primary, textTransform: "uppercase", marginBottom: 4 },
  dateTitle: { fontSize: 20, fontWeight: "700", color: colors.text, marginBottom: spacing.md },
  section: { marginTop: spacing.lg, paddingTop: spacing.lg, borderTopWidth: 1, borderTopColor: colors.border },
  sectionTitle: { fontSize: 16, fontWeight: "700", color: colors.text, marginBottom: spacing.sm },
  timeText: { fontSize: 18, fontWeight: "600", color: colors.primary },
  napCard: { backgroundColor: colors.background, borderRadius: borderRadius.md, padding: spacing.md, marginBottom: spacing.sm },
  napHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.sm },
  napTitle: { fontSize: 16, fontWeight: "700", color: colors.text },
  ...
  resultsBox: { backgroundColor: colors.primary + "10", borderRadius: borderRadius.sm, padding: spacing.sm, marginTop: spacing.sm },
  resultsTitle: { fontSize: 13, fontWeight: "700", color: colors.primary, marginBottom: 4 },
  resultText: { fontSize: 13, color: colors.text, marginBottom: 2 },
  ...
  observationsBox: { backgroundColor: colors.card, borderRadius: borderRadius.sm, padding: spacing.sm, marginTop: spacing.sm, borderLeftWidth: 3, borderLeftColor: colors.primary },
  observationsLabel: { fontSize: 12, fontWeight: "700", color: colors.primary, marginBottom: 4 },
  observationsText: { fontSize: 13, color: colors.text },
  commentsSection: { marginTop: spacing.sm },
  commentsLabel: { fontSize: 13, fontWeight: "700", color: colors.textSecondary, marginBottom: spacing.xs },
  commentsInput: { backgroundColor: colors.card, borderRadius: borderRadius.sm, padding: spacing.sm, fontSize: 14, color: colors.text, borderWidth: 1, borderColor: colors.border, minHeight: 80, textAlignVertical: "top" },
  commentButtons: { flexDirection: "row", gap: spacing.sm, marginTop: spacing.sm },
  saveButton: { flex: 1, backgroundColor: colors.primary, paddingVertical: spacing.sm, borderRadius: borderRadius.sm, alignItems: "center" },
  saveButtonText: { fontSize: 14, fontWeight: "600", color: "#FFFFFF" },
  cancelButton: { flex: 1, backgroundColor: colors.background, paddingVertical: spacing.sm, borderRadius: borderRadius.sm, alignItems: "center", borderWidth: 1, borderColor: colors.border },
  cancelButtonText: { fontSize: 14, fontWeight: "600", color: colors.text },
  commentsText: { fontSize: 14, color: colors.text, fontStyle: "italic" },
  generalCommentsButton: { backgroundColor: colors.background, borderRadius: borderRadius.sm, padding: spacing.sm, borderWidth: 1, borderColor: colors.border },
  wakingsSection: { marginTop: spacing.md },
  wakingsTitle: { fontSize: 15, fontWeight: "700", color: colors.text, marginBottom: spacing.sm },
  wakingCard: { backgroundColor: colors.background, borderRadius: borderRadius.sm, padding: spacing.sm, marginBottom: spacing.xs },
  wakingTitle: { fontSize: 14, fontWeight: "600", color: colors.text, marginBottom: 4 },
  backButton: { flexDirection: "row", alignItems: "center", padding: spacing.lg, gap: spacing.sm },
