    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.sectionCard}>
          <Text style={styles.sectionCardTitle}>☀️ Acordou às {wakeUpTime}</Text>
        ...
        {naps.map((nap, index) => {
          ...
          const prevWakeTime = index === 0 ? wakeUpTime : (naps[index - 1]?.wakeUpTime || wakeUpTime);
          const sleepWindowRealized = nap.startTryTime && prevWakeTime ? calcTimeDiff(prevWakeTime, nap.startTryTime) : null;
            ...
            <View key={nap.id} style={styles.expandableCard}>
              ...
              {isExpanded && (
                <View style={styles.expandableContent}>
                  <View style={styles.napDataBlock}>
                    <Text style={styles.napBlockTitle}>Horários</Text>
                    <View style={styles.napTimeRow}>
                      <Text style={styles.napTimeLabel}>Levado ao quarto:</Text>
                      <Text style={styles.napTimeValue}>{nap.startTryTime}</Text>
                    </View>
                    <View style={styles.napTimeRow}>
                      <Text style={styles.napTimeLabel}>Dormiu às:</Text>
                      <Text style={styles.napTimeValue}>{nap.fellAsleepTime || "—"}</Text>
                    </View>
                    <View style={styles.napTimeRow}>
                      <Text style={styles.napTimeLabel}>Acordou às:</Text>
                      <Text style={styles.napTimeValue}>{nap.wakeUpTime || "—"}</Text>
                    </View>
                  </View>
                  
                  {(sleepWindowRealized !== null || timeToSleep !== null || sleepDuration !== null) && (
                    <View style={styles.resultsBlock}>
                      <Text style={styles.resultsBlockTitle}>Resultados:</Text>
                      
                      {sleepWindowRealized !== null && (
                        <View style={styles.resultRow}>
                          <Text style={styles.resultLabel}>Janela de sono realizada:</Text>
                          <Text style={styles.resultValue}>{minutesToHM(sleepWindowRealized)}</Text>
                          <Text style={styles.resultDetail}>({prevWakeTime} → {nap.startTryTime})</Text>
                        </View>
                      )}
                      
                      {timeToSleep !== null && (
                        <View style={styles.resultRow}>
                          <Text style={styles.resultLabel}>Tempo para adormecer:</Text>
                          <Text style={styles.resultValue}>{minutesToHM(timeToSleep)}</Text>
                          <Text style={styles.resultDetail}>({nap.startTryTime} → {nap.fellAsleepTime})</Text>
                        </View>
                      )}
                      ...
                      {sleepDuration !== null && (
                        <View style={styles.resultRow}>
                          ...
                          <Text style={styles.resultValue}>{minutesToHM(sleepDuration)}</Text>
                          <Text style={styles.resultDetail}>({nap.fellAsleepTime} → {nap.wakeUpTime})</Text>
                        </View>
                      )}
                    </View>
                  )}
            ...
            <View style={styles.reportSummary}>
              ...
              <View style={styles.reportGrid}>
                ...
                <View style={styles.reportGridItem}>
                  ...
                  <Text style={styles.reportGridLabel}>Total emI'll improve the routine visualization for the consultant profile by adding:
1. Day of the week display (in Portuguese, uppercase)
2. Better organized results with calculations for each nap
3. Proper formatting for time durations (1h, 1h15, 45min)
...
function RoutineDetailScreen({ isConsultant, baby, routine: initialRoutine, dayNumber, onBack, showErr }: {
  ...
  const dayOfWeek = getDayOfWeek(routine.date);
  const dateBR = formatDateToBR(routine.date);
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.dateHeader}>
          <Text style={styles.dayOfWeek}>{dayOfWeek}</Text>
          <Text style={styles.dateDisplay}>{dateBR}</Text>
        </View>
        ...
        {naps.map((nap, index) => {
          ...
          let previousWakeTime = wakeUpTime;
          if (index > 0 && naps[index - 1].wakeUpTime) {
            previousWakeTime = naps[index - 1].wakeUpTime!;
          }
          
          const sleepWindowRealized = nap.startTryTime && previousWakeTime ? calcTimeDiff(previousWakeTime, nap.startTryTime) : null;
          const timeToSleep = nap.startTryTime && nap.fellAsleepTime ? calcTimeDiff(nap.startTryTime, nap.fellAsleepTime) : null;
          const sleepDuration = nap.fellAsleepTime && nap.wakeUpTime ? calcTimeDiff(nap.fellAsleepTime, nap.wakeUpTime) : null;
          ...
          return (
            <View key={nap.id} style={styles.expandableCard}>
              ...
              {isExpanded && (
                <View style={styles.expandableContent}>
                  <Text style={styles.napSubtitle}>Horários</Text>
                  
                  <Text style={styles.fieldLabel}>Levado ao quarto</Text>
                  <View style={styles.readOnlyBox}>
                  ...
                  {sleepWindowRealized !== null && (
                    <View style={styles.resultRow}>
                      <Text style={styles.resultLabel}>Janela de sono realizada:</Text>
                      <Text style={styles.resultValue}>{minutesToHM(sleepWindowRealized)}</Text>
                      <Text style={styles.resultDetail}>({previousWakeTime} → {nap.startTryTime})</Text>
                    </View>
                  )}
...
const styles = StyleSheet.create({
  ...
  reportGridItem: { flex: 1, minWidth: "45I'll improve the routine visualization for the consultant profile by adding:
1. Day of the week display (in Portuguese, uppercase)
2. Better organized results section with calculations
3. Formatted time durations (1h, 1h15, 45min)
...
/**
 * Converts minutes to formatted duration string (e.g., "1h15", "45min", "2h")
 */
function minutesToHM(minutes: number): string {
  const h = Math.floor(minutes / 60);
  const m = minutes % 60;
...
/**
 * Calculates time difference in minutes between two time strings (HH:MM)
 */
function calcTimeDiff(start: string, end: string): number {
  const [sh, sm] = start.split(":").map(Number);
  const [eh, em] = end.split(":").map(Number);
...
/**
 * Gets the day of the week in Portuguese (uppercase)
 */
function getDayOfWeek(dateStr: string): string {
  const daysOfWeek = [
    "DOMINGO",
...
function RoutineDetailScreen({ isConsultant, baby, routine: initialRoutine, dayNumber, onBack, showErr }: {
  ...
  const dayOfWeek = getDayOfWeek(routine.date);
  const formattedDate = formatDateToBR(routine.date);
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {naps.map((nap, index) => {
          ...
          // Calculate results
          const prevNapWakeTime = index === 0 ? wakeUpTime : (naps[index - 1]?.wakeUpTime || null);
          const sleepWindowRealized = prevNapWakeTime && nap.startTryTime ? calcTimeDiff(prevNapWakeTime, nap.startTryTime) : null;
          const timeToFallAsleep = nap.startTryTime && nap.fellAsleepTime ? calcTimeDiff(nap.startTryTime, nap.fellAsleepTime) : null;
          const sleepDuration = nap.fellAsleepTime && nap.wakeUpTime ? calcTimeDiff(nap.fellAsleepTime, nap.wakeUpTime) : null;
            ...
            <View key={nap.id} style={styles.expandableCard}>
              ...
              {isExpanded && (
                <View style={styles.expandableContent}>
                  ...
                  {/* RESULTS SECTION */}
                  {(sleepWindowRealized !== null || timeToFallAsleep !== null || sleepDuration !== null) && (
                    <View style={styles.resultsSection}>
                      ...
                      {sleepWindowRealized !== null && (
                        <View style={styles.resultRow}>
                          ...
                          <Text style={styles.resultDetail}>
                            ({prevNapWakeTime} → {nap.startTryTime})
                          </Text>
                        </View>
                      )}
                      ...
                      {timeToFallAsleep !== null && (
                        <View style={styles.resultRow}>
                          <Text style={styles.resultLabel}>Tempo para adormecer:</Text>
                          <Text style={styles.resultValue}>{minutesToHM(timeToFallAsleep)}</Text>
                          <Text style={styles.resultDetail}>
                    ...
                    <View style={styles.expandableContent}>
                      ...
                      <View style={styles.resultsSection}>
                        <View style={styles.resultRow}>
                          ...
                          <Text style={styles.resultValue}>{minutesToHM(duration)}</Text>
                        </View>
                      </View>
                    </View>
                  )}
