...
/**
 * Converts minutes to formatted string (e.g., "1h15", "45min", "2h")
 */
...
/**
 * Calculates time difference in minutes between two HH:MM times
 */
...
/**
 * Gets the day of the week in Portuguese uppercase from a date string (YYYY-MM-DD)
 */
      ...
      >
        ...
        {totalDays === 0 ? (
          ...
          Object.keys(routinesByDay).map((dayKey) => {
            ...
            // Calculate day of week if routine exists
            const dayOfWeek = routine ? getDayOfWeek(routine.date) : null;
            ...
            return (
              ...
              >
                <View style={styles.routineCardHeader}>
                  <View>
                    {dayOfWeek && <Text style={styles.routineDayOfWeek}>{dayOfWeek}</Text>}
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {/* DAY OF WEEK AND DATE HEADER */}
        <View style={styles.dayHeader}>
          <Text style={styles.dayOfWeekText}>{dayOfWeek}</Text>
          <Text style={styles.dateText}>{formatDateToBR(routine.date)}</Text>
          <Text style={styles.dayNumberText}>Dia {dayNumber}</Text>
        </View>
        ...
        {naps.map((nap, index) => {
          ...
          // Calculate previous wake up time (for janela realizada)
          let previousWakeUpTime = wakeUpTime;
          if (index > 0) {
            const prevNap = naps[index - 1];
            previousWakeUpTime = prevNap.wakeUpTime || wakeUpTime;
          }
          
          // Calculate results
          const janelaRealizada = nap.startTryTime ? calcTimeDiff(previousWakeUpTime, nap.startTryTime) : null;
          const tempoAdormecer = (nap.startTryTime && nap.fellAsleepTime) ? calcTimeDiff(nap.startTryTime, nap.fellAsleepTime) : null;
          const duracaoSono = (nap.fellAsleepTime && nap.wakeUpTime) ? calcTimeDiff(nap.fellAsleepTime, nap.wakeUpTime) : null;
              ...
              {isExpanded && (
                <View style={styles.expandableContent}>
                  ...
                  <View style={styles.resultsSection}>
                    ...
                    {janelaRealizada !== null && (
                      <View style={styles.resultRow}>
                        <Text style={styles.resultLabel}>Janela de sono realizada pela mãe:</Text>
                        <Text style={styles.resultValue}>{minutesToHM(janelaRealizada)}</Text>
                        <Text style={styles.resultRange}>({previousWakeUpTime} → {nap.startTryTime})</Text>
                      </View>
                    )}
                    ...
                    {tempoAdormecer !== null && (
                      <View style={styles.resultRow}>
                        ...
                        <Text style={styles.resultValue}>{minutesToHM(tempoAdormecer)}</Text>
                        <Text style={styles.resultRange}>({nap.startTryTime} → {nap.fellAsleepTime})</Text>
                      </View>
                    )}
                    ...
                    {duracaoSono !== null && (
                      <View style={styles.resultRow}>
                        ...
                        <Text style={styles.resultValue}>{minutesToHM(duracaoSono)}</Text>
                        <Text style={styles.resultRange}>({nap.fellAsleepTime} → {nap.wakeUpTime})</Text>
                      </View>
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  addButtonText: { fontSize: 16, fontWeight: "600", color: "#Vou melhorar a exibição da rotina no perfil da consultora, adicionando o dia da semana e organizando melhor os resultados das sonecas com cálculos precisos.
      ...
      >
        ...
        {totalDays === 0 ? (
          ...
          Object.keys(routinesByDay).map((dayKey) => {
            ...
            // Calculate day of week
            const dayOfWeek = routine ? getDayOfWeek(routine.date) : "";
...
const styles = StyleSheet.create({
  ...
  welcomeTitle: { fontSize: 22, fontWeight: "bold", color: colors.text, textAlign: "center", marginBottom: 12 },
  welcomeText: { fontSize: 16, color: colors.textSecondary, textAlign: "center", lineHeight: 24, marginBottom: 12 },
  motherInfoBox: { backgroundColor: colors.backgroundAlt, borderRadius: 12, padding: 12, marginTopVou melhorar a exibição da rotina no perfil da consultora, adicionando o dia da semana e organizando melhor os resultados das sonecas com cálculos precisos.
      ...
      >
        ...
        {totalDays === 0 ? (
          ...
          Object.keys(routinesByDay).map((dayKey) => {
            ...
            const dayOfWeek = routine ? getDayOfWeek(routine.date) : "";
            const dateDisplay = routine ? formatDateToBR(routine.date) : "";
            ...
            return (
              ...
              >
                <View style={styles.routineCardHeader}>
                  <View>
                    {dayOfWeek && <Text style={styles.routineDayOfWeek}>{dayOfWeek}</Text>}
                    {dateDisplay && <Text style={styles.routineDate}>{dateDisplay}</Text>}
                    <Text style={styles.routineDayNumber}>Dia {dayNumber}</Text>
                  </View>
                  <View style={[styles.routineStatusBadge, { backgroundColor: statusColor }]}>
                    <Text style={styles.routineStatusText}>{routineStatus}</Text>
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {naps.map((nap, index) => {
          ...
          const janelaRealizada = nap.startTryTime && prevWakeTime ? calcTimeDiff(prevWakeTime, nap.startTryTime) : null;
          const tempoAdormecer = nap.startTryTime && nap.fellAsleepTime ? calcTimeDiff(nap.startTryTime, nap.fellAsleepTime) : null;
          const duracaoSono = nap.fellAsleepTime && nap.wakeUpTime ? calcTimeDiff(nap.fellAsleepTime, nap.wakeUpTime) : null;
                  ...
                  <View style={styles.resultsSection}>
                    ...
                    {janelaRealizada !== null && (
                      <View style={styles.resultRow}>
                        <Text style={styles.resultLabel}>Janela de sono realizada pela mãe:</Text>
                        <Text style={styles.resultValue}>{minutesToHM(janelaRealizada)}</Text>
                        <Text style={styles.resultRange}>({prevWakeTime} → {nap.startTryTime})</Text>
                      </View>
                    )}
...
const styles = StyleSheet.create({
  ...
  routineCardHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 },
  routineDayOfWeek: { fontSize: 14, fontWeight: "bold", color: colors.primary, marginBottom: 2 },
  routineDate: { fontSize: 13, color: colors.textSecondary, marginBottom: 2 },
  routineDayNumber: { fontSize: 16, fontWeight: "600", color: colors.text },
