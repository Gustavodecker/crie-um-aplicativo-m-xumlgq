...
/**
 * Utility function to calculate time difference and format it in a friendly way.
 * Returns null if either time is missing.
 * Format: "1h15" if >= 1 hour, "45min" if < 1 hour
 */
function diffTime(start: string | null | undefined, end: string | null | undefined): string | null {
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {naps.map((nap, index) => {
          ...
          // Calculate nap results using diffTime utility
          const previousNap = index > 0 ? naps[index - 1] : null;
          const windowStart = index === 0 ? wakeUpTime : previousNap?.wakeUpTime;
          const windowEnd = nap.startTryTime;
          
          const janelaRealizada = diffTime(windowStart, windowEnd);
          const tempoAdormecer = diffTime(nap.startTryTime, nap.fellAsleepTime);
          const duracaoSono = diffTime(nap.fellAsleepTime, nap.wakeUpTime);
          
          const hasResults = janelaRealizada && tempoAdormecer && duracaoSono;
                ...
                <View style={styles.expandableContent}>
                  ...
                  {hasResults && (
                    <View style={styles.resultsCard}>
                      ...
                      <Text style={styles.resultsText}>Janela realizada: {janelaRealizada}</Text>
                      <Text style={styles.resultsText}>Tempo para adormecer: {tempoAdormecer}</Text>
                      <Text style={styles.resultsText}>Duração do sono: {duracaoSono}</Text>
                    </View>
                  )}
...
const styles = StyleSheet.create({
  ...
  I'll add the nap calculation results display to the consultant's routine view. Let me create a utility function and add the results display within the nap rendering section.
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {naps.map((nap, index) => {
          ...
          // Calculate window start time (for first nap: wakeUpTime, for others: previous nap wakeUpTime)
          const windowStartTime = index === 0 ? wakeUpTime : (naps[index - 1]?.wakeUpTime || null);
          
          // Calculate the three metrics using diffTime
          const janelaRealizada = diffTime(windowStartTime, nap.startTryTime);
          const tempoAdormecer = diffTime(nap.startTryTime, nap.fellAsleepTime);
          const duracaoSono = diffTime(nap.fellAsleepTime, nap.wakeUpTime);
          
          // Only show results if all three values are available
          const showResults = janelaRealizada && tempoAdormecer && duracaoSono;
...
const styles = StyleSheet.create({
  ...
  modalButtonText: { fontSize: 16, fontWeight: "600", color: "#FFFFFF" },
  modalHint: { fontSize: 13, color: colors.textSecondary, textAlign: "center", marginBottom: 8, fontStyle: "italic" },
  successI'll add the nap calculation results display to the consultant's routine detail screen. This will calculate and show the three metrics (Janela realizada, Tempo para adormecer, Duração do sono) for each nap.
...
/**
 * Utility function to calculate time difference and format it in a friendly way.
 * Returns formatted string like "1h15" or "45min" or null if times are invalid.
 */
function diffTime(start: string | null | undefined, end: string | null | undefined): string | null {
  if (!start || !end) return null;
            ...
            <View key={nap.id} style={styles.expandableCard}>
              ...
              {isExpanded && (
                <View style={styles.expandableContent}>
                  ...
                  {showResults && (
                    <View style={styles.napResultsBox}>
                      <Text style={styles.napResultsTitle}>Resultados:</Text>
                      <Text style={styles.napResultText}>Janela realizada: {janelaRealizada}</Text>
                      <Text style={styles.napResultText}>Tempo para adormecer: {tempoAdormecer}</Text>
                      <Text style={styles.napResultText}>Duração do sono: {duracaoSono}</Text>
                    </View>
                  )}
