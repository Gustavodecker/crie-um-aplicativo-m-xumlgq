...
export default function ReportsLandscapeScreen() {
  ...
  const headerPeriodText = `${formatDateToBR(report.startDate)} até ${formatDateToBR(report.endDate)}`;
  const weeklyAverageText = minutesToHM(Math.round(report.weeklyAverage));
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={styles.headerCard}>
          ...
          <Text style={styles.headerPeriod}>{headerPeriodText}</Text>
        </View>
        ...
        <View style={styles.summaryContainer}>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text style={styles.summaryValue}>{weeklyAverageText}</Text>
          </View>
        </View>
        ...
        <View style={styles.sectionContainer}>
          ...
          {report.dailyEvolution.map((day, index) => {
            ...
            const indicatorLabel = day.indicator === "green" ? "Ótimo" : day.indicator === "yellow" ? "Bom" : "Atenção";
            const dayTitle = `Dia ${index + 1}`;
            const dayDateText = formatDateToBR(day.date);
              ...
              <View key={day.date} style={[styles.dayCard, { borderLeftColor: indicatorColor, borderLeftWidth: 4 }]}>
                <View style={styles.dayHeader}>
                  <View>
                    ...
                    <Text style={styles.dayDate}>{dayDateText}</Text>
                  </View>
                  <View style={[styles.indicatorBadge, { backgroundColor: indicatorColor }]}>
                    <Text style={styles.indicatorText}>{indicatorLabel}</Text>
                  </View>
                </View>
                    ...
                    {(routine.naps || []).length > 0 && (
                      <View style={styles.subsection}>
                        ...
                        {(routine.naps || []).map((nap, napIndex) => {
                          ...
                          const napTitle = `Soneca ${napIndex + 1}`;

                          return (
                            <View key={nap.id} style={styles.napCard}>
                              <Text style={styles.napTitle}>{napTitle}</Text>
                          ...
                          {(routine.nightSleep.wakings || []).length > 0 && (
                            <View style={styles.wakingsContainer}>
                              ...
                              {(routine.nightSleep.wakings || []).map((waking, wakingIndex) => {
                                ...
                                const wakingLabel = `${wakingIndex + 1}º despertar:`;
                                const wakingValue = `${waking.startTime} - ${waking.endTime} (${minutesToHM(duration)})`;
                                return (
                                  <View key={waking.id} style={styles.wakingRow}>
                                    <Text style={styles.wakingLabel}>{wakingLabel}</Text>
                                    <Text style={styles.wakingValue}>{wakingValue}</Text>
                                  </View>
                                );
                              })}
                            </View>
...
const styles = StyleSheet.create({
  ...
  headerCard: { backgroundColor: colors.card, borderRadius: 12, padding: 16, marginBottom: 16, alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  headerTitle: { fontSize: 16, fontWeight: "600", color: colors.textSecondary, marginBottom: 4 },
  headerPeriod: { fontSize: 18, fontWeight: "bold", color: colors.primary },
  summaryContainer: { flexDirection: "row", flexWrap: "wrap", gap: 12, marginBottom: 24 },
  summaryCard: { flex: 1, minWidth: 150, backgroundColor: colors.card, borderRadius: 12, padding: 16, alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 3, elevation: 2 },
  summaryLabel: { fontSize: 13, color: colors.textSecondary, textAlign: "center", marginBottom: 8 },
  ...
  dayCard: { backgroundColor: colors.card, borderRadius: 12, padding: 16, marginBottom: 16, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  dayHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
  dayTitle: { fontSize: 18, fontWeight: "bold", color: colors.text },
  dayDate: { fontSize: 14, color: colors.textSecondary, marginTop: 2 },
  indicatorBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  indicatorText: { fontSize: 12, fontWeight: "bold", color: "#fff" },
  ...
  dayValue: { fontSize: 14, fontWeight: "600", color: colors.text },
  subsection: { marginTop: 16 },
  subsectionTitle: { fontSize: 16, fontWeight: "bold", color: colors.primary, marginBottom: 8 },
  napCard: { backgroundColor: colors.background, borderRadius: 8, padding: 12, marginBottom: 8 },
  napTitle: { fontSize: 14, fontWeight: "bold", color: colors.text, marginBottom: 8 },
  napRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 },
  napLabel: { fontSize: 13, color: colors.textSecondary, flex: 1 },
  napValue: { fontSize: 13, fontWeight: "500", color: colors.text },
  napValueHighlight: { fontSize: 13, fontWeight: "bold", color: colors.primary },
  wakingsContainer: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: colors.border },
  wakingsTitle: { fontSize: 13, fontWeight: "bold", color: colors.text, marginBottom: 6 },
  wakingRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 },
  wakingLabel: { fontSize: 12, color: colors.textSecondary },
  wakingValue: { fontSize: 12, fontWeight: "500", color: colors.text },
  dayTotals: { marginTop: 16, paddingTop: 16, borderTopWidth: 2, borderTopColor: colors.border },
  ...
  totalValueHighlight: { fontSize: 16, fontWeight: "bold", color: colors.primary },
});
